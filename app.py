...
def load_result(analysis_id: str):
    ...
    if os.path.exists(path):
        try:
            ...
            # Refresh blockchain summary in case chain has grown since last save
...
def run_analysis(df: pd.DataFrame, filename: str, protected: str, outcome: str,
    ...
    result["_report_txt"] = generate_txt_report(result)
    analysis_store[analysis_id] = result
    # NOTE: save_result() is intentionally NOT called here.
    # It is called in the /analyze route AFTER blockchain keys are attached,
    # so persisted results always contain file_hash and blockchain data.
    return result
...
def analyze():
    ...
    justi_chain.add_analysis_block(
        ...
        outcome_col  = outcome,
        row_count    = len(df),
        dir_val      = float(result["metrics"][0]["value"]) if result.get("metrics") else None,
        spd_val      = float(result["metrics"][1]["value"]) if result.get("metrics") else None,
        eod_val      = float(result["metrics"][2]["value"]) if result.get("metrics") else None,
    )
    result["file_hash"]        = file_hash
    result["blockchain"]       = justi_chain.summary()
    result["blockchain_block"] = justi_chain.latest.to_dict()

    # ── Persist result to disk NOW (after blockchain keys are attached)
...
def api_analyze():
    ...
    try:
        ...
        # Attach blockchain record for API responses too
        file_bytes_api = b""  # already consumed; hash from df
        api_file_hash = hash_dataframe(df)
        justi_chain.add_file_block(
        ...
        result["file_hash"]  = api_file_hash
        result["blockchain"] = justi_chain.summary()
        save_result(result["analysis_id"], result)
        append_history({
            "analysis_id":  result["analysis_id"],
            "filename":     file.filename,
            "file_hash":    api_file_hash,
